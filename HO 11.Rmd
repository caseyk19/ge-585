...
## Next steps:
# need to look at how uncertainties interact with each other (covariance terms and correlations); go through analysis and answer 5 q's and then choose one of three options
...
Gauss.Pred <- function(x) {
  ...
  for(z in 0:length(mean)){
    ci[1,z]<- mean[z]+ 1.96*SD[z]
    ci[2,]<- mean
    ci[3,z]<- mean[z]- 1.96*SD[z]
  }
  ci
}
```
...
> The Gaussian approximation is larger than the full Monte Carlo ensemble.  Across all sources of uncertainty the MC ensemble it a tighter/closer fit.  Similar to the full ensemble, the Gaussian approximation is has narrower 95% standard deviation when predicting species decline rather than population increases.  This is observed at times when the forecast sharply declines or inclines, respectively.
...
> When the NMC is at 50 the parameter uncertainty is about the samle as the MC when NMC=1000, but when NMC is at 10 the parameter uncertainty is much smaller than when NMC=1000
...
> For relative variance: out-of-sample the NMC=50 has similar proportions of variance compared to the NMC=1000 MC.  The IC is smallest followed by parameter uncertainty.  Driver variance is greater for NMC=1000 farther into time.  The Process variance for NMC=50 has a different pattern than NMC=1000 in that it peaks and declines at different times, and is proportionally a bit larger than in the NMC=1000 MC.  The Random Effect variance is the largest variance for both NMC=50 and NMC=1000.  When the NMC=10 the proportion of variance is different.  The IC variance is still the smallest. The Parameter variance is still the next smallest, but is not constant throughout time, instead it is smaller in the beginning and end and has two increases at 40 and 45.  The Driver variance is highest between 45 and 50 unlike the other MC runs which saw this variance increase with time.  Next, the process variance is largest the largest source of variance, not random effect like the other MC runs.  
> The relative variance in-sample is very similar between the NMC=1000 and NMC=10 MCs, with slight differences in proportion of driver and parameter variance.  The former is larger in NMC=1000 while the latter is proportionally greater in NMC=50.  The NMC in-sample relative variance has smaller parameter variance and has a driver variance that has distinct peaks.  These peaks indicate sensitivity of the drivers at specific times during prediction.

#Comparing NMC=1000 Gaussian to NMC=50, and NMC=10 MCs
>For IC the NMC=1000 and NMC=50 have similar standard deviations while the NMC=10 is smaller than both of them.
> For Parameter uncertainty the NMC=50 and NMC=10 are about the same size as the gaussian approximation uncertainties.
> For Driver uncertainty NMC=10 and NMC=50 have a much smaller uncertainty, particularly the upper bound, compared to NMC=1000 gaussian approximation.  Additionally the driver uncertainty does not increase as much as the gaussian as we would expect the driver uncertainty to do.
> For process uncertainty NMC=10 and NMC=50 have less process error uncertainty overall, and are more sensitive to predicted increase or decreases.  The NMC=10 and NMC=50 MC output had sharper peaks and dips compared to the gaussian approximation standard deviations.  Additionally the process error does not decrease as much for NMC=10 compared to the gaussian.  This contradicts what we would expect which is process error decrease over time.
> The random effect uncertainty is larger for the gaussian approximation than for the MC output for NMC=10.  This could be due to the decrease in iterations which reduced the amount of Random effects introduced over time.  Random effect uncertainty for NMC=50 has a larger upper bound confidence interval than the gaussian standard deviations.  The lower confidence interval has a different pattern than the gaussian because it responds more strongly/sensitively to increase or decreases.

#Comparing Gaussian results across NMC=1000, NMC=50, and NMC=10

> The NMC=1000 has slightly larger IC uncertainty than NMC=50.  The parameter and driver uncertainty is about the same between NMC=50 and NMC=1000.  The driver uncertainty for both meets expectations of increasing over time.  The process error uncertainty declines over time for both NMC runs, as expected.  The NMC=1000 process error uncertainty follows the predictive output more closely in pattern than when NMC=50.  The peaks and dips follow te pattern of the output more closely whereas for NMC=50 the uncertainty does not follow the pattern f output exactly.  Instead it has peaks or dips that are larger in NMC=50.  This suggest that a smaller NMC is more sensitive because is does not iterate as much so it improves less.  Finally, for the random effect uncertainty the standard deviations are smaller for NMC=50.  This indicates that having more iterations increases the amount of uncertainty introduced from random effects.

> The Gaussian approximation IC and parameter uncertainty is about the same between NMC=1000 and NMC=10.  However, the driver uncertainty is larger for the NMC=1000 especially above the forecast and at later time periods as one would expect.  For process error uncertainty the NMC=10 does not decline over time as much as NMC=1000 does, and as one would expect.  The NMC=10 pattern had larger  peaks and dips compared to NMC=1000.  This indicates that less iterations makes a model more sensitive, larger standard deviations, to approximate increases and decreases.  The standard deviation on random effects is smaller for NMC=10 compared to NMC=1000.  This could indicate that having fewer iterations introduces less uncertainty/smaller standard deviations.
