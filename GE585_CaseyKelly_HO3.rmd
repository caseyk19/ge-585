...
```{r}
seedata<- read.csv('GE585_RISEEDLING.csv', header=TRUE) #reading in data
treecount <-seedata$TREECOUNT #taking out desired variables
specode<- seedata$SPCD
treespec<- na.omit(as.data.frame(cbind(seedata$TREECOUNT, seedata$SPCD))) #creating new data frame with desired variables and removing rows with NAs
colnames(treespec)<- c('Treecount', 'specode') #assigning column names
ABC<-as.data.frame(aggregate(rank(Treecount)~specode, treespec, sum ))
ABC<- ABC[order(ABC$specode, decreasing=TRUE),]
colnames(ABC)<- c('specode','Treecount')
sort(ABC$specode, decreasing=TRUE) #ordering dataframe with spec code decreasing

plot(ABC$specode, ABC$treecount, log='y',
     xlab="Species of Tree Seedling Code Ranked",
     ylab="Log(abundance of Tree Seedlings)",
     main="Rank vs. Log(abundance) Tree Seedling counts in RI",
     type='b',
     pch=24
     )
```
...
> Was not able to scrape the table off the web.  Below is how I would go about trying to complete this question.

```
nu <- function(x){as.numeric(as.character(x))}  ## simple function to convert data to numeric
flux_html <- getURL("http://fluxnet.fluxdata.org/sites/site-list-and-pages/")  ## grab raw html
flux_table <-readHTMLTable("http://fluxnet.fluxdata.org/sites/site-list-and-pages/", header=TRUE, skip.rows = 1)
flux_table <- as.data.frame(flux_table)
ctrycode<-substring(flux_table$siteID, 1, 2)
ccflux_table<-cbind.data.frame(c('ctrycode','flux_table'))
ccflux_table<-aggregate.data.frame(rank(sitename)~ctrycode, sitename, count )
sort(ccflux_table$ctrycode, decreasing=FALSE) #ordering dataframe with spec code decreasing
...
```{r}
myCode = readLines("GE585_CaseyKelly_HO3.Rmd")  ## read unstructured text
...
```{r}
myCode = readLines("GE585_CaseyKelly_HO3.Rmd")  ## read unstructured text
x = grep('^#',myCode)    ## returns the line numbers that begin with comment character #
myCode[x]
```
...
```
system("wget http://thredds.daac.ornl.gov/thredds/dodsC/ornldaac/1220/mstmip_driver_global_hd_climate_tair_2004_v1.nc4 ") #cant find this file using the url

## open the netCDF file
wlef = nc_open("mstmip_driver_global_hd_climate_tair_2004_v1.nc4")
print(wlef)    ## metadata

lat<- ncvar_get(wlef, "lat") # get lat data
lon<- ncvar_get(wlef, "lon") # get lon data
yr<- ncvar_get(wlef, "time") # get time data


###Boston coordinates are 42.3601° N, 71.0589° W
which(lat=="71.0589" & lon="42.3601") #finding which cells are Boston

start = which(yr == 2004) #only cells from 2004
wlef = nc_open("mstmip_driver_global_hd_climate_tair_2004_v1.nc4")
TA = ncvar_get(wlef,"TA",start) #At this point I would grab the cells of Boston data (identified in prior steps) using the time parameterstart
plot(TA,type = 'l') #plotting Boston air quality data form 2004
nc_close(wlef)
...
```{r}
plot(time,EVI, type='b', main="EVI of Willow Creek vs. Time (2012)",
     ylab="Enhanced Vegetation Index",
     xlab= "Time (2012)")
```
> Was not able to get the CO2 flux observation so I was not able to compare.  Looking at the EVI plot I postulate that CO2 would decrease as EVI increases and increases as the EVI decreases.  Since vegetation utilizes CO2 in photosyntehsis, periods of plant productivity would decrease the amount of CO2 detected by the towers.  Since the summer months provide ample sunlight, these periods of time would record the greatest Enhanced Vegetation Index values and the lowest CO2 flux values.
...
recent <- function(x){
  ...
  fia_table = readHTMLTable(fia_html)[[3]]    ## We're interested in the 3rd table on this webpage
  update = as.Date(fia_table[,"Last Modified Date"]) 
  result<- fia_table[which(update > "2018/01/01"),]
  return(result)
  }  ## one function to update

* * * * 1 /home/GE585_CaseyKelly_HO3/update.sh 
