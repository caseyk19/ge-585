...
> The simple ecosystem ensemble had the largest spread which increased over time, which is to be expected due to process error uncertainty increasing over time. The Resampling particle filter has the next largest spread around and the non-resampling particle filter has the smallest LAI spread. Both particle filters were more accurate than the simple ecosystem ensemble up to November. One would expect the non-resampling PF to be overconfident and have a smaller spread because each analysis downweighting and upweighting is applied which can lead to convergence a smaller number of ensemble members with the highest likelihood. This reduction in effective number of particles in the MCMC results in the tuning of errors (i.e. process) and low forecast uncertainty. This leads to the filter divergence observed in both PF after November and in the non-resampling PF after September. The Resampling PF likely ensemble members are upweighted, cloned, and have process errors attributed to them so there is some underlying source of uncertainty. This approach is meant to avoid the filter divergence and ensemble convergence that occurs in non-resampling PFs. Because we did not apply process error to each ensemble error in the forecast step we still observe this filter divergence despite the model having high accuracy and a non-convergence.
...
> Comparing the initial conditions, drivers, and state variables I assessed the time series plots of each in addition to covariance plots of each against LAI. The uncertainty for the simple ecosystem ensemble model were higher than both PF for all fluxes and state variables. Bleaf had high positive correlation with LAI. The other initial conditions, BSOM (soil organic matter) and BWOOD (wood biomass), had a less defined relationship with LAI. GPP (gross primary productivity), RA (soil respiration), NPPW (net primary production of wood), and NPPl (net primary production of leaves) had positive relationship with LAI. As LAI increases so do these fluxes and variables. There is a large variance in how these covary with LAI. Net ecosystem production (NEP) did not covary with LAI and had a constant zero slope relationship. Rh and CWD likewise did not covary with LAI, but had larger variance in this relationship. Looking at the individual time-series plots of these fluxes and state variables one observes larger confidence intervals that in the PF projections. Bwood, Bleaf, BSOM, litter, CWD, and LAI relatively smooth with gradual decreases/increases through time. However GPP, NEP, Ra, NPPw, NPPl, and Rh show larger interannual variance that manifest in sharp peaks and dips. This could reflect sensitivity to interannual changes in our drivers, especially temperature which has a similar time-series pattern to these state variables.

> Looking at the parameter distributions from the resample PF, we observe that the standard deviations for parameter distributions decreases from priors to the posteriors. The initial conditions parameters (tau.leaf, tau.stem, and tau.soil) and process error parameters (falloc 1, 2, and 3) all had decreased standard deviations from their prior distributions to posterior distributions. This reflects the resampling PFs ability to constrain the other fluxes and error propagation in the model, especially tau.leaf and falloc 2. Additionally, the state variable parameters also have decreasing parameter distribution standard deviation, especially for state variables with high covariance with LAI such as CWD and SLA. smaller confidence intervals it has for forecasted LAI and in the reduced standard deviations in parameter distributions. The resample PF constrains the uncertainty from drivers and parameters that we would expect to increase over time. Unlike the simple ensemble model but not to the extent of the non-resampled PF, this projection actually reduces the standard deviations in parameter distributions across almost every parameter.

> The narrowing of the confidence from the non-resampled PF exhibit this projections ability to greatly constrain all other fluxes and state variables. Due to the weighting and non-resampling of this projection we observe that the model increases in certainty and thus constraining the driver uncertainty, process error uncertainty, and state variables to the extent that we observe filter divergence The LAIr observations and their own uncertainty intervals in November are not capture by this PF due to this overconfidence and constraint on their fluxes and variables.


3) Which parameters were most constrained by LAI? Does this make sense?

>The posterior and prior parameter distributions for the resampled PF demonstrate how this methodology applies upweighting, which results in smaller standard deviations on parameters. This methodology results in the cloning of more likely ensemble members which increases the frequency of those members occuring and decreases the variance in ensemble members. The prior and posterior distribution plot show that certain values of each parameter are sample at an increased frequency and the standard deviation decreases across parameters. These changes reflect which parameter are most constrained by LAI because they will experience the largest reductions in standard deviations and increased frequency of sampling. For resampled OF these parameters are The parameters most constrained by LAI in the resampling PF are CWD (coarse woody debris), alpha (light use efficiency), SLA (specific leaf area), tau.leaf (process error leaf biomass), and falloc 2 & 3 (priors on wood and leaf NPP).

> It makes sense that these parameters are most constrained by LAI. With regards to CWD, a low tree mortality rate means that there are more healthy trees with leaves on their branches in the forest and therefore a higher LAI. With regards to light use efficiency it makes sense that more leaves (higher LAI) would increase light use efficiency and that a reduced LAI would reduce this parameter. Next, it makes sense that LAI constrain SLA because SLA is the ratio of lead area to dry mass which makes SLA and LAI directly related leaf metrics. Another constrained parameter was the process error on leaf biomass. It makes sense that this would be constrained by LAI because a larger leaf with inherently have more biomass than a smaller leaf because it has a increased capacity to photosynthesize. One would expect increased LAI and leaf biomass to be highly constrained. The process error of leaf biomass, which represents the uncertainty in forecasted leaf biomass, will therefore be constrained by LAI. Finally the priors on wood and leaf NPP (falloc 2 and 3) are highly constrained by LAI. It makes sense for these parameters to be constrained by LAI, because the surface area of leaves and the canopy area are important drivers of NPP. More, larger leaves increases the leaf and woody NPP potential of trees.
...
> I fixed every ensemble member to have the mean of each parameter for each update while maintaining different initial conditions. Comparing the projection of the resampled PF with parameter uncertainty vs the projection without parameter uncertainty we observe that the confidence intervals are greater.  This increased model uncertainty reflects the projection loosened constraint on process error, which increases over time, and other parameter.  The parameter distributions are much flatter and wider due to the updated parameters simply being the mean value of that parameter.  Compared to the resampling with a parameter distribution and uncertainty, the weighting is less effective because past ensemble members that were upweighted greatly no longer existed and therefore the model did not reduced uncertainty by weighting more likely ensemble members. 
