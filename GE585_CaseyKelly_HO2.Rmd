...
n<- matrix(n0, nrow=4, ncol=30)
for(t in 2:NT){
  for(i in 1:4){
  n[i,t] = n[i,t-1] + r[i]*n[i,t-1]*(1-n[i,t-1]/K)
  }
}

par(mfrow = c(2, 2))
for (j in 1:4){
plot(time,n[j,],ylim=c(0,12),lwd=3,type='l',
     bty='l',cex.lab=1.5, col=j,
     xlab="Time",ylab="Population Size")
  title(paste('Intrinsic growth rate', r[j]))
  }
```
> For an instrinsic growth rate of 1.95 the trajectoryhas a decreasing variance over time with the population moving above and below 10.

> For the intrinsic growth rate of 2.05 the trajectory has more variance that does not visuallly appear to decrease over time.  The population moves above and below 10 in larger steps that in the smaller growth rate (population decreases and increases to 8 and 12 as a opposed to 9 and 11 in the later model).

> The intrinsic growth rate of 3.5 does not have a decreasing or homooskedasticity variance observed in the two smaller growth rate models.  The trajectory in this model is more difficult to determine because the variance is heteroskedastic.

> The trajectoryof population with an intrinsic growth rate of 2.8 is the most difficult to determine because it is heteroskedasticity and has the greatest visual variance in size.
...
par(mfrow=c(1,1))
x = seq(10,200,by=10)
plot(x,dpois(x,100),type='l')  		
abline(v=100)				          	
lines(x,dpois(x,150),col=2)			
abline(v=150,col=2)
lines(x,dnorm(x,50,20),col=3)	
abline(v=50,col=3)

plot(x,ppois(x,100,1),type='l')
abline(v=100)
lines(x,ppois(x,150,1),col=2)
abline(v=150,col=2)
lines(x,pnorm(x,50,2),col=3)
abline(v=50,col=3)

p = seq(0,1,by=0.01)
plot(p,qnorm(p,150,20),type='l',ylim=range(x))   
abline(h=150)						
lines(p,qnorm(p,100,20),col=2)
abline(h=100,col=2)
lines(p,qnorm(p,50,40),col=3)
abline(h=50,col=3)


n = c(10,20,30,40)	# sequence of sample sizes
for(i in 1:4){			# loop over these sample sizes
  hist(rpois(n[i],150),main=n[i],probability=TRUE,breaks=20)  
  #here breaks defines number of bins in the histogram
  lines(x,dpois(x,150),col=2)
}
```
...
x = rlnorm(10000,0,1)
y = sin(x)
hist(x,main="Log-Normal distribution",breaks=40)
abline(v=quantile(x,c(0.025,0.5,0.975)),lty=c(2,1,2),lwd=3,col="orange")
abline(v=mean(x),col="red",lwd=3,lty=3)
legend("topright",
       lty=c(2,1,3),
       ...
       cex=.75)
quantile(x,c(0.025,0.5,0.975)) #median and CI for x
mean(x) #mean of x
...
legend("topleft",
       ...
       col=c("orange", "orange", "red"),
       cex=.75)
quantile(y,c(0.025,0.5,0.975)) #median and CI for y
mean(y) #mean of y
```
Parameter error
---------------
...
par(mfrow=c(2,1))
hist(rE,main="Samples of R",breaks=40)
abline(v=quantile(rE,c(0.025,0.5,0.975)),lty=c(2,1,2),lwd=3,col="orange")
abline(v=mean(rE),col="red",lwd=3,lty=3)
legend("topright",
       lty=c(2,1,3),
       c("95% CI", "Median", "Mean"),
       col=c("orange", "orange", "red"),
...
hist(KE,main="Samples of K",breaks=40)
abline(v=quantile(KE,c(0.025,0.5,0.975)),lty=c(2,1,2),lwd=3,col="orange")
abline(v=mean(KE),col="red",lwd=3,lty=3)
legend("topright",
       lty=c(2,1,3),
       c("95% CI", "Median", "Mean"),
       col=c("orange", "orange", "red"),
...
```{r}
par(mfrow=c(1,1))
plot(time, n[12,], type='l', col=1, xlab="Time", ylab="Trajectories", main="Trajectories through Time")
lines(time, n[14,], type='l', col=2)
lines(time, n[524,], type='l', col=3)
lines(time, n[16,], type='l', col=4)
lines(time, n[87,], type='l', col=5)
lines(time, n[94,], type='l', col=6)
lines(time, n[200,], type='l', col=7)
lines(time, n[567,], type='l', col=8)
lines(time, n[734,], type='l', col=9)
lines(time, n[874,], type='l', col=10)
```
6. Plot a histogram of your population forecast at time = 15.
```{r}
hist(n[,15], main="Population Forecast at Time 15", breaks=10, xlab="Time 15")
```
...
```{r}
n.stats<-as.matrix(apply(n,2,quantile,c(0.025,0.5,0.975)))

n.med<-as.matrix(apply(n,2,quantile,c(0.5))) #median trajectory through time
n.yhi<- as.matrix(apply(n,2,quantile,c(0.975))) #97.5 CI of trajectories through time
n.ylo<- as.matrix(apply(n,2,quantile,c(0.025))) #25% CI of trajectories through time
plot(time, n.med, type='l', col=1)
lines(time, n.yhi, lty=2, lwd=3, col="orange")
lines(time, n.ylo, lty=2, lwd=3, col="orange")
legend("topright",
       lty=2,
       c("95% CI", "25% CI"),
       col=c("orange", "orange", "red"),
...
n.yhi<- as.matrix(apply(n,2,quantile,c(0.975))) #97.5% CI of trajectories through time
n.ylo<- as.matrix(apply(n,2,quantile,c(0.025))) #25% CI of trajectories through time
plot(time, n.med, type='l', col=1, ylab="median trajectory values", main="Median Trajectories of Lognormal distribution through Time")
lines(time, n.yhi, lty=2, lwd=3, col="orange")
lines(time, n.ylo, lty=2, lwd=3, col="orange")
legend("topleft",
       lty=2,
...
n.med<-as.matrix(apply(n,2,quantile,c(0.5))) #median trajectory through time
n.yhi<- as.matrix(apply(n,2,quantile,c(0.975))) #95% CI of trajectories through time
n.ylo<- as.matrix(apply(n,2,quantile,c(0.025))) #25% CI of trajectories through time
plot(time, n.med, type='l', col=1, ylab="median trajectory values", main="Median Trajectories of Lognormal Distribution through Time with r=1.95")
lines(time, n.yhi, lty=2, lwd=3, col="orange")
lines(time, n.ylo, lty=2, lwd=3, col="orange")
legend("topright",
       lty=2,
...
n.yhi<- as.matrix(apply(n,2,quantile,c(0.975))) #95% CI of trajectories through time
n.ylo<- as.matrix(apply(n,2,quantile,c(0.025))) #25% CI of trajectories through time
plot(time, n.med, type='l', col=1, ylab="median trajectory values", main="Median Trajectories of Lognormal Distribution through Time with r=2.05")
lines(time, n.yhi, lty=2, lwd=3, col="orange")
lines(time, n.ylo, lty=2, lwd=3, col="orange")
legend("topright",
       lty=2,
...
.stats<-as.matrix(apply(n,2,quantile,c(0.025,0.5,0.975)))

n.med<-as.matrix(apply(n,2,quantile,c(0.5))) #median trajectory through time
n.yhi<- as.matrix(apply(n,2,quantile,c(0.975))) #95% CI of trajectories through time
n.ylo<- as.matrix(apply(n,2,quantile,c(0.025))) #25% CI of trajectories through time
plot(time, n.med, type='l', col=1, ylab="median trajectory values", main="Median Trajectories of Lognormal Distribution through Time with r=2.8")
lines(time, n.yhi, lty=2, lwd=3, col="orange")
lines(time, n.ylo, lty=2, lwd=3, col="orange")
legend("topright",
       lty=2,
